...
class Work(models.Model):
    ...
    def __str__(self):
        return "%s (%s, Dauer: %s)" % (self.dateFrom.strftime("%d.%m.%y"),
                                       self.category,
    ...
    def calculateDuration(self):
        if self.dateTo:
            if self.type == Types.HOLIDAY:
                daygenerator = [self.dateFrom + datetime.timedelta(x)
                                for x in range(
                                    (self.dateTo - self.dateFrom).days + 1)
                                if (self.dateFrom +
                                    datetime.timedelta(x)).weekday() < 5]
                return sum(1 for _ in daygenerator) * 60 * 60
